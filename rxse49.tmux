...
main() {
    ...
    # Status bar
    set "status" "on"
    set status-style "fg=$green,bg=$black"
    ...
    set message-style "fg=$cyan,bg=$black"
    ...
    # Windows
    setw window-status-style "fg=${dim_blue},bg=${black}"
    ...
    local user
    user="$(get_tmux_option "@rxse49_user" "")"
    readonly user

    # Shows hostname of the computer the tmux session is run on
    local host
    host="$(get_tmux_option "@rxse49_host" "")"
    readonly host

    # Date and time command: follows the date UNIX command structure
    local date_time
    date_time="$(get_tmux_option "@rxse49_date_time" "")"
    readonly date_time

    # Shows truncated current working directory
    local directory
    directory="$(get_tmux_option "@rxse49_directory" "")"

    local disable_active_window_menu
    disable_active_window_menu="$(get_tmux_option "@rxse49_disable_active_window_menu" "")"

    local show_current_program
    show_current_program="$(get_tmux_option "@rxse49_show_current_program" "")"
    readonly show_current_program

    local window_directory
    window_directory="$(get_tmux_option "@rxse49_show_pane_directory" "")"
    readonly window_directory

    local window_separator
    window_separator="$(get_tmux_option "@rxse49_window_separator" "")"
    readonly window_separator

    local default_window_behavior
    default_window_behavior="$(get_tmux_option "@rxse49_default_window_behavior" "")"
    ...
    # active_window_color="$(get_tmux_option "@rxse49_active_window_color" "")"
    # readonly active_window_color

    # Transparency enabling for status bar
    local bar_bg_disable
    bar_bg_disable="$(get_tmux_option "@rxse49_bar_bg_disable" "")"
    readonly bar_bg_disable

    # Transparent option for status bar
    local bar_bg_disabled_color_option
    bar_bg_disabled_color_option="$(get_tmux_option "@rxse49_bar_bg_disabled_color_option" "0")"
    readonly bar_bg_disabled_color_option

    # Shows hostname of the computer the tmux session is run on
    local only_windows
    only_windows="$(get_tmux_option "@rxse49_only_windows" "")"
    ...
    status_left_prepend_section="$(get_tmux_option "@rxse49_status_left_prepend_section" "")"
    readonly status_left_prepend_section
    #
    # after the plugin's left section
    local status_left_append_section
    status_left_append_section="$(get_tmux_option "@rxse49_status_left_append_section" "")"
    readonly status_left_append_section
    # Before the plugin's right section
    local status_right_prepend_section
    status_right_prepend_section="$(get_tmux_option "@rxse49_status_right_prepend_section" "")"
    readonly status_right_prepend_section
    #
    # after the plugin's right section
    local status_right_append_section
    status_right_append_section="$(get_tmux_option "@rxse49_status_right_append_section" "")"
    ...
    current_window_icon="$(get_tmux_option "@rxse49_current_window_icon" "")"
    readonly current_window_icon

    local current_session_icon
    current_session_icon="$(get_tmux_option "@rxse49_session_icon" "")"
    readonly current_session_icon

    local username_icon
    username_icon="$(get_tmux_option "@rxse49_username_icon" "")"
    readonly username_icon

    local hostname_icon
    hostname_icon="$(get_tmux_option "@rxse49_hostname_icon" "󰒋")"
    readonly hostname_icon

    local date_time_icon
    date_time_icon="$(get_tmux_option "@rxse49_date_time_icon" "󰃰")"
    readonly date_time_icon

    local current_folder_icon
    current_folder_icon="$(get_tmux_option "@rxse49_folder_icon" "")"
    ...
    window_status_separator="$(get_tmux_option "@rxse49_window_status_separator" "  ")"

    # This setting does nothing by itself, it enables the 2 below it to toggle the simplified bar
    local prioritize_windows
    prioritize_windows="$(get_tmux_option "@rxse49_prioritize_windows" "")"

    # Allows the user to set a min width at which most of the bar elements hide, or
    local user_window_width
    user_window_width="$(get_tmux_option "@rxse49_width_to_hide" "")"

    # A number of windows, when over it, the bar gets simplified
    local user_window_count
    user_window_count="$(get_tmux_option "@rxse49_window_count" "")"

    # Custom window status that goes between the number and the window name
    local custom_window_sep="#[fg=$dim_blue]#I#[fg=$dim_blue,]$window_separator#[fg=$dim_blue]#W"
    local custom_window_sep_current="#I#[fg=$yellow,bg=""]$window_separator#[fg=$yellow,bg=""]#W"

    local right_separator
    right_separator="$(get_tmux_option "@rxse49_right_separator" "  ")"

    local left_separator
    left_separator="$(get_tmux_option "@rxse49_left_separator" "  ")"

    local field_separator
    # NOTE: Don't remove
    field_separator="$(get_tmux_option "@rxse49_field_separator" " | " )"
    ...
    show_window_in_window_status="#[fg=$dim_blue]#I#[fg=$dim_blue,]$left_separator#[fg=$dim_blue]#W"
    ...
    local show_user
    readonly show_user="#[fg=$dim_blue]#(whoami)#[fg=$bright_cyan]$right_separator#[fg=$bright_cyan]$username_icon"

    local show_host
    readonly show_host="$spacer#[fg=$text]#H#[fg=$bright_cyan]$right_separator#[fg=$bright_cyan]$hostname_icon"

    local show_date_time
    readonly show_date_time=" #[fg=$bright_green]$date_time#[fg=$bright_cyan]$right_separator#[fg=$bright_cyan]$date_time_icon "
    ...
    show_directory_in_window_status="#I$left_separator#[fg=$yellow,bg=""]#{b:pane_current_path}"

    local show_directory_in_window_status_current
    show_directory_in_window_status_current="#I$left_separator#[fg=$yellow,bg=""]#{b:pane_current_path}"
    ...
    if [[ "$bar_bg_disable" == "on" ]]; then
        ...
        show_window_in_window_status="#[fg=$dim_blue,bg=$bar_bg_disabled_color_option]#I#[fg=$dim_blue,bg=$bar_bg_disabled_color_option]$left_separator#[fg=$dim_blue,bg=$bar_bg_disabled_color_option]#W"
        ...
        show_directory_in_window_status="#[fg=$dim_blue,bg=$bar_bg_disabled_color_option]#I#[fg=$dim_blue,bg=$bar_bg_disabled_color_option]$left_separator#[fg=$dim_blue,bg=$bar_bg_disabled_color_option]#{b:pane_current_path}"
        ...
        set window-status-style "fg=$dim_blue,bg=$bar_bg_disabled_color_option"
    ...
    setw clock-mode-colour "${red}"
    setw mode-style "fg=${yellow}"
